...
while($semester_row=mysqli_fetch_array($semester_query)){
    ...
    <select id="course_yearSelect" class="form-control" name="course_year_section" required >
        <option>Select</option>
		<option>--Select--</option>
...
while($course_row=mysqli_fetch_array($course_query)){
	$courseData = trim($course_row['course'].' '.$course_row['year_section']);
	?>

	<option data-course="<?=$courseData?>"><?=$courseData?></option>
	<?php } ?>
    </select>
</div>

<div id="subjectContainerz" class="control-group">
    <label class="control-label" for="inputPassword">Subject:</label>
    <div class="controls">
        <select id="subjectSelect" type="text" name="subject_code" class="form-control">
		<option>--Select--</option>
		<?php $subj_query=mysqli_query($conn,"select * from subject")or die(mysqli_error());
while($subject_row=mysqli_fetch_array($subj_query)){
	?>

	<option data-course="<?=$subject_row['course']?>"><?php echo $subject_row['subject_title']; ?></option>
	<?php } ?>
	</select>
